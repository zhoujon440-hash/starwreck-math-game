...
| BC-006 | 旧项目总登记表与场景、道具、机制、UI 正式域清单的名称或状态存在 267 条差异；此前导入器虽挂接正式包路径，却仍从旧登记表生成语义字段。 | 场景 V1.0、道具 V3.0、MECH/UI/FX/DANGER V2.0、G01 V3.0 及人物 V2.1 的正式行分别是各域字段权威；旧总登记表对非人物域仅为 `cross_check_only`。 | 导入器改为八域显式字段映射，重建全部 488 行；`CAT-ASSET-FORMAL-ROW-CONTENT` 与 `CAT-ASSET-FORMAL-ROW-UNIQUE` 打开正式 ZIP 中 CSV/XLSX 逐行核对。完整差异见 `docs/review/ASSET_REGISTRY_VS_FORMAL_CATALOG_DIFF.md`。 |
...
- 正式域 CSV/XLSX 与旧登记表仍保留 267 条可解释差异。当前版本优先级明确，因此不阻断；若正式包自身出现重复 ID、缺行或无法唯一定位，逐行门禁会直接失败，不能退回旧登记表补值。
- DANGER V2.0 的 76 项正式 CSV 列为星球、危险名称、类型、应用场景、分批、独立设计板、确认状态和冻结版本，并不含“风险预兆/软失败表现/安全恢复节点”列。目录不会伪造这些正式行字段；危险软失败恢复继续由章节存档数据和已登记结构化适配契约单独逐项校验。
