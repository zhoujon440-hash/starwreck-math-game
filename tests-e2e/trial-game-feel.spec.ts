...
mechanics.forEach((mechanic, index) => {
  test(`${mechanic.sceneId} moves from scene to ${mechanic.type} device interaction and visible world change`, async ({ page }, info) => {
    ...
    await outputScreenshot(page, info, `${String(index + 1).padStart(2, '0')}-${mechanic.type}-scene-before.png`)
    ...
    await expect(panel).toHaveAttribute('data-in-world-mechanic', 'true')
    await expect(panel).not.toHaveAttribute('aria-modal', 'true')
    await expect(stage.locator('.trial-mechanic-panel')).toHaveCount(0)
    await expect(page.locator('.topbar')).toHaveCount(0)
    await outputScreenshot(page, info, `${String(index + 1).padStart(2, '0')}-${mechanic.type}-object-focus.png`)
    await makeOneRealMove(page, panel, mechanic.type)
    await expect(panel).toHaveAttribute('data-mechanic-status', 'partial')
    await outputScreenshot(page, info, `${String(index + 1).padStart(2, '0')}-${mechanic.type}-interaction-in-world.png`)
    ...
    await outputScreenshot(page, info, `${String(index + 1).padStart(2, '0')}-${mechanic.type}-world-after.png`)
