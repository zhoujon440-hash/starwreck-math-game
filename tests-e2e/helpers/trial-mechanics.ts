...
import { dragMechanicToken } from './reliable-drag'
...
const drag = async (page: Page, panel: Locator, token: string, slot: string): Promise<void> => {
...
export const solveTrialMechanic = async (
  ...
  if (type === 'rotating-circuit') {
    ...
    for (const [token, slot] of [['pressure', 'timeline-1'], ['patch', 'timeline-2'], ['repress', 'timeline-3']]) await drag(page, panel, token, slot)
    ...
    for (const [token, slot, rotations] of [['fragment-a', 'star-slot-a', 1], ['fragment-b', 'star-slot-b', 3], ['fragment-c', 'star-slot-c', 2]] as const) {
      ...
      await drag(page, panel, token, slot)
    }
    ...
    for (const [token, slot] of [['weight-1', 'right-far'], ['weight-2', 'right-near'], ['weight-3', 'left-near']]) await drag(page, panel, token, slot)
  } else {
    const placements = {
    ...
    for (const [token, slot] of Object.entries(placements)) await drag(page, panel, token, slot)
  }
